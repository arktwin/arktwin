...
class TransformSpec extends AnyFunSpec with Matchers:
  ...
  given Equality[Rotation] =
    case (a: EulerAngles, b: EulerAngles) =>
      ...
      f(a).zip(f(b)).forall(summon[Equality[Double]].areEqual.tupled)
  ...
  given Equality[Option[Vector3]] =
    case (Some(a), Some(b)) =>
      summon[Equality[Vector3]].areEqual(a, b)
    case (None, None) =>
      true
    case _ =>
      false
  ...
  given Equality[Transform] =
    case (a: Transform, b: Transform) =>
      ...
      summon[Equality[Vector3]].areEqual(a.globalScale, b.globalScale) &&
      summon[Equality[Rotation]].areEqual(a.localRotation, b.localRotation) &&
      summon[Equality[Vector3]].areEqual(a.localTranslation, b.localTranslation) &&
      summon[Equality[Option[Vector3]]].areEqual(a.localTranslationSpeed, b.localTranslationSpeed)
    case _ =>
      false

  describe("Transform"):
    describe("apply and normalize"):
      it("denormalizing value equals to the original value with the same config"):
        for
          axisConfig <-
            Seq(
